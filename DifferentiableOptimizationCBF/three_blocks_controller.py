...
from DiffOptCBF.solver.cbfqp_solver import CBFQPSolver
from DiffOptCBF.exp_utils import change_quat_format, get_link_config, get_Q_mat

# load Julia functions
j = julia.Julia()
create_arm = j.include("dc_utils/create_arm_ellipsoid.jl")
three_blocks_exp_setup = j.include("dc_utils/three_blocks_exp_setup.jl")
get_cbf_three_blocks = j.include("dc_utils/get_cbf_three_blocks.jl")


class ThreeBlocksDiffOptQPController:
    def __init__(self):
        # load URDF file
    ...
    def controller(self, t, q, dq):
        ...
        if not self.initialized:
            ...
            self.p_end = np.array([[0.7], [0.05], [0.1]])
            self.R_end = get_R_end_from_start(0, 0, 0, self.R_start)
